...
class Updater:
    ...
    def fetch_current_season(self, request_new: bool):
        ...
        self.json_data["fixtures"][self.current_season] = self.fetch_fixtures_data(
            self.current_season, request_new
        )
        self.json_data["standings"][self.current_season] = self.fetch_standings_data(
            self.current_season, request_new
        )
        self.json_data["fantasy"][self.current_season] = self.fetch_fantasy_data(
            self.current_season, request_new
        )
    ...
    def build_dataframes(self, n_seasons: int, display_tables: bool = False):
        ...
        self.data.standings.build(
            self.json_data, self.current_season, n_seasons, display=display_tables
        )
        # Fixtures for the whole season for each team
        self.data.fixtures.build(
            self.json_data, self.current_season, display=display_tables
        )
