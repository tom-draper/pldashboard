<script lang="ts">
	...
	import { toAlias, playedMatchdayDates, getCurrentMatchday, toHyphenatedName } from './team';
	import { toTitleCase, slugAlias } from './format';
	...
	function switchTeam(newTeam: Team) {
		data.slug = slugAlias(toHyphenatedName(newTeam));
		data.team = toTitleCase(data.slug.replace(/-/g, ' ')) as Team;
		data.title = `Dashboard | ${data.team}`;
		// Overwrite values from new team's perspective using same data
		data.currentMatchday = getCurrentMatchday(data.data, data.team);
		data.playedDates = playedMatchdayDates(data.data, data.team);
...
<div id="team">
	<Nav team={data.team} teams={data.teams} {switchTeam} />
			...
			<div class="page-content">
					...
					<div class="row-left position-no-badge">
						<div class="circles-background-container">
							<svg class="circles-background">
								...
								/>
								<circle cx="170" cy="170" r="140" stroke-width="0" fill="var(--{data.slug})" />
								<circle cx="300" cy="320" r="170" stroke-width="0" fill="var(--{data.slug})" />
							</svg>
						</div>
						<div class="position-central">
							{data.data.standings[data.team][data.data._id].position}
						</div>
					</div>
					<div class="row-right fixtures-graph row-graph">
						<h1 class="lowered">Fixtures</h1>
						<div class="graph mini-graph mobile-margin">
							<FixturesGraph data={data.data} team={data.team} {mobileView} />
						</div>
					</div>
				</div>
				...
				<div class="row multi-element-row">
					<div class="row-left form-details">
						<CurrentForm data={data.data} currentMatchday={data.currentMatchday} team={data.team} />
						<TableSnippet
							data={data.data}
					...
					<div class="row-right">
						<NextGame data={data.data} team={data.team} {switchTeam} />
					</div>
				</div>
