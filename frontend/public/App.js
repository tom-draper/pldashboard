...
const NextGame = create_ssr_component(($$result, $$props, $$bindings, slots) => {
	...
	onMount(() => {
		if (data.upcoming[fullTeamName].nextTeam != null) {
			oppTeam = data.upcoming[fullTeamName].nextTeam.replace(" FC", "").toLowerCase().replace(/ /g, "-");
		}
	});
...
<div class="${"spider-opp-team-selector"}">
  <div class="${"spider-opp-team-btns"}" id="${"spider-opp-teams"}">${each(data.teamNames, teamName => {
		...
		? `<button class="${"spider-opp-team-btn"}">${escape(teamName.replace(" FC", ""))}</button>`
		: ``}`;
	})}</div></div>`;
});
...
const Team = create_ssr_component(($$result, $$props, $$bindings, slots) => {
	...
	return `${($$result.head += `${($$result.title = `<title>${escape(fullTeamName.replace(' FC', ''))}</title>`, "")}<meta name="${"description"}" content="${"Premier League Statistics Dashboard"}">`, "")}

${validate_component(Router, "Router").$$render($$result, {}, {}, {
		default: () => {
			return `<div class="${"header"}" style="${"background-color: var(--" + escape(team) + ");"}"><div class="${"main-link title no-decoration"}" style="${"color: var(--" + escape(team + '-secondary') + ");"}">${escape(fullTeamName.replace(' FC', ''))}</div></div>
