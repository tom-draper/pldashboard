...
function getFormStarTeams(data, team, matchdays) {
	...
	for (let matchday of matchdays) {
		formStarTeams.push(data.form[team][matchday].beatStarTeam
		? "star-team"
		: "");
	}
...
const CurrentForm = create_ssr_component(($$result, $$props, $$bindings, slots) => {
	...
	? `<div class="${"current-form-row"}"><div class="${"icon pos-0 " + escape(formIcons[0], true) + " " + escape(formStarTeams[0], true)}"></div>
    <div class="${"icon pos-1 " + escape(formIcons[1], true) + " " + escape(formStarTeams[1], true)}"></div>
    <div class="${"icon pos-2 " + escape(formIcons[2], true) + " " + escape(formStarTeams[2], true)}"></div>
    <div class="${"icon pos-3 " + escape(formIcons[3], true) + " " + escape(formStarTeams[3], true)}"></div>
    <div class="${"icon pos-4 " + escape(formIcons[4], true) + " " + escape(formStarTeams[4], true)}"></div></div>
  <div class="${"current-form-row"}"><div class="${"icon-name pos-0"}">${escape(formInitials[0])}</div>
    <div class="${"icon-name pos-1"}">${escape(formInitials[1])}</div>
    <div class="${"icon-name pos-2"}">${escape(formInitials[2])}</div>
    <div class="${"icon-name pos-3"}">${escape(formInitials[3])}</div>
    <div class="${"icon-name pos-4"}">${escape(formInitials[4])}</div></div>`
	: ``}
${currentMatchday != undefined
	? `<div class="${"current-form"}">Current form:
    ${currentMatchday != null
		? `${escape((data.form[fullTeamName][currentMatchday].formRating5 * 100).toFixed(2))}%`
		: `None`}</div>`
	: ``}`;
});
...
const TableSnippet = create_ssr_component(($$result, $$props, $$bindings, slots) => {
    ...
    ${each(Array(tableSnippet.rows.length), (_, i) => {
      ...
      ${i == tableSnippet.teamTableIdx
			? `
        <div class="${"table-row this-team"}" style="${"background-color: var(--" + escape(team, true) + ");"}"><div class="${"table-element table-position this-team"}" style="${"color: var(--" + escape(team, true) + "-secondary);"}">${escape(tableSnippet.rows[i].position)}</div>
          <div class="${"table-element table-team-name this-team"}" style="${"color: var(--" + escape(team, true) + "-secondary);"}">${escape(tableSnippet.rows[i].name)}</div>
          <div class="${"table-element table-gd this-team"}" style="${"color: var(--" + escape(team, true) + "-secondary);"}">${escape(tableSnippet.rows[i].gd)}</div>
          <div class="${"table-element table-points this-team"}" style="${"color: var(--" + escape(team, true) + "-secondary);"}">${escape(tableSnippet.rows[i].points)}</div>
        </div>`
			: `
...
const SeasonComplete = create_ssr_component(($$result, $$props, $$bindings, slots) => {
	...
	return `<div class="${"next-game-prediction row-graph"}"><div class="${"next-game-season-complete"}"><h1 class="${"next-game-title-text"}">${escape(data.currentSeason)}/${escape(data.currentSeason + 1)} SEASON COMPLETE
    </h1></div></div>`;
});
...
const NextGameStats = create_ssr_component(($$result, $$props, $$bindings, slots) => {
	...
	let { data, fullTeamName, currentMatchday } = $$props;
	if ($$props.data === void 0 && $$bindings.data && data !== void 0) $$bindings.data(data);
	if ($$props.fullTeamName === void 0 && $$bindings.fullTeamName && fullTeamName !== void 0) $$bindings.fullTeamName(fullTeamName);
	if ($$props.currentMatchday === void 0 && $$bindings.currentMatchday && currentMatchday !== void 0) $$bindings.currentMatchday(currentMatchday);
	...
	return `<div class="${"next-game-prediction row-graph"}" style="${"border: 6px solid var(--" + escape(data.upcoming[fullTeamName].nextTeam.toLowerCase().replace(/ /g, '-'), true) + ");"}"><div class="${"next-game-title"}" style="${"background-color: var(--" + escape(data.upcoming[fullTeamName].nextTeam.toLowerCase().replace(/ /g, '-'), true) + ");"}"><h1 class="${"next-game-title-text"}" style="${"color: var(--" + escape(data.upcoming[fullTeamName].nextTeam.toLowerCase().replace(/ /g, '-'), true) + "-secondary);"}">Next Game: ${validate_component(Link, "Link").$$render(
		...
		{
			default: () => {
				return `<div class="${"no-decoration"}">${escape(data.upcoming[fullTeamName].nextTeam)}</div>`;
			}
		}
	)}<span class="${"parenthesis"}">(</span>${escape(data.upcoming[fullTeamName].atHome ? "Home" : "Away")}<span class="${"parenthesis"}">)</span></h1></div>

  <div class="${"next-game-values"}"><div class="${"predictions-and-logo"}"><div class="${"next-game-logo opposition-badge"}" style="${"background-image: url('" + escape(data.logoURLs[data.upcoming[fullTeamName].nextTeam], true) + "')"}"></div>
      <div class="${"predictions"}"><div class="${"next-game-item"}">Current form:
          ${currentMatchday != null
	? `<b>${escape(data.form[data.upcoming[fullTeamName].nextTeam][currentMatchday].formRating5)}%</b>`
	: `None`}</div>
        <div class="${"next-game-item"}">Score prediction
          <br>
          <a class="${"predictions-link"}" href="${"/predictions"}"><b>${escape(Math.round(data.upcoming[fullTeamName].prediction.homeGoals))} - ${escape(Math.round(data.upcoming[fullTeamName].prediction.awayGoals))}</b></a>
          <br>
          </div></div></div>
    <div class="${"past-results"}">${data.upcoming[fullTeamName].prevMatches.length == 0
	...
	: `<div class="${"next-game-item prev-results-title"}">Previous Results</div>`}
      ...
      ${each(data.upcoming[fullTeamName].prevMatches, prevMatch => {
		return `<div class="${"next-game-item " + escape(prevMatch.result, true)}"><div class="${"past-result"}"><div class="${"home-team"}">${escape(prevMatch.homeTeam)}</div>
          ...
          <div class="${"past-result-date"}">${escape(new Date(prevMatch.date).toLocaleDateString("en-us", {
			weekday: "long",
			year: "numeric",
			month: "short",
			day: "numeric"
...
const NextGame = create_ssr_component(($$result, $$props, $$bindings, slots) => {
	...
	? `${data.upcoming[fullTeamName].nextTeam == null
		...
		: `${validate_component(NextGameStats, "NextGameStats").$$render($$result, { data, currentMatchday, fullTeamName }, {}, {})}`}`
	: ``}`;
});
...
const SeasonStats = create_ssr_component(($$result, $$props, $$bindings, slots) => {
	...
	return `<div class="${"season-stats"}"><div class="${"season-stat goals-per-game"}"><div class="${"season-stat-value"}">${escape(data.seasonStats[fullTeamName].xG)}
      <div class="${"season-stat-position ssp-" + escape(rank.xG, true)}" id="${"ssp1"}"${add_attribute("this", ssp1, 0)}>${escape(rank.xG)}</div></div>
    ...
    <div class="${"season-stat conceded-per-game"}"><div class="${"season-stat-value"}">${escape(data.seasonStats[fullTeamName].xC)}
        <div class="${"season-stat-position ssp-" + escape(rank.xC, true)}" id="${"ssp2"}"${add_attribute("this", ssp2, 0)}>${escape(rank.xC)}</div></div>
    ...
    <div class="${"season-stat clean-sheet-ratio"}"><div class="${"season-stat-value"}">${escape(data.seasonStats[fullTeamName].cleanSheetRatio)}
        <div class="${"season-stat-position ssp-" + escape(rank.cleanSheetRatio, true)}" id="${"ssp3"}"${add_attribute("this", ssp3, 0)}>${escape(rank.cleanSheetRatio)}</div></div>
...
const TeamsFooter = create_ssr_component(($$result, $$props, $$bindings, slots) => {
  ...
  <div class="${"teams-footer-bottom"}">${lastUpdated != null
	? `<div class="${"last-updated svelte-niomtf"}">Last updated: ${escape(lastUpdated)} UTC</div>`
	: ``}
    ...
    <div class="${"footer-bottom"}"><div class="${"created-by footer-text-colour"}">Created by Tom Draper</div>
      <div class="${"version footer-text-colour"}">v2.0</div></div></div>
</div>`;
});
...
const Team = create_ssr_component(($$result, $$props, $$bindings, slots) => {
	...
	onMount(() => {
		...
		fetchData$1("http://127.0.0.1:5000/teams").// fetchData("https://pldashboard.herokuapp.com/teams")
		then(json => {
...
${validate_component(Router, "Router").$$render($$result, {}, {}, {
		default: () => {
			return `<div class="${"header"}" style="${"background-color: var(--" + escape(team, true) + ");"}"><div class="${"main-link title no-decoration"}" style="${"color: var(--" + escape(team + '-secondary', true) + ");"}">${escape(fullTeamName)}</div></div>

  ${data != undefined
			? `<div class="${"page-content"}"><div class="${"row"}">${`<div class="${"row-left position-no-badge"}"><div class="${"circles-background-container"}"><svg class="${"circles-background"}"><circle cx="${"300"}" cy="${"150"}" r="${"100"}" stroke-width="${"0"}" fill="${"var(--" + escape(team, true) + "-secondary)"}"></circle><circle cx="${"170"}" cy="${"170"}" r="${"140"}" stroke-width="${"0"}" fill="${"var(--" + escape(team, true) + ")"}"></circle><circle cx="${"300"}" cy="${"320"}" r="${"170"}" stroke-width="${"0"}" fill="${"var(--" + escape(team, true) + ")"}"></circle></svg></div>
  ...
  ${data != undefined
      ...
      <div class="${"predictions-container"}"><div class="${"predictions"}">${data.predictions != null
              ...
              ${each(predictions, pred => {
							return `<button class="${"prediction-container " + escape(pred.colour, true)}"><div class="${"prediction prediction-item"}"><div class="${"prediction-label"}">Predicted:</div>
